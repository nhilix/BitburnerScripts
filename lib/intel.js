import { formatMoney, formatDuration, formatNumberShort, disableLogs } from '/lib/util.js';
 ...
 * Requires: Singularity API for ns.travelToCity BN-4.1 **/
        ...
        while (ns.getPlayer().money > moneyThreshold) {
            ...
            if (previousInt != ns.getPlayer().intelligence) {
                ...
                ns.print(`Level Up: Int ` + currentInt + (justStarted ? ` Partial` : ` Full`) + ` Level in `
                    + formatDuration(ns,duration/1000) + ` & ` + formatNumberShort(ns,tripsPerLevel) + ` Travels`);
