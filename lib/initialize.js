const root = '/hacking/root-runner.js',
    rootHack = '/hacking/rootHack.js',
    leaf = '/hacking/leaf-runner.js',
...
export async function main(ns) {
    ...
    */
    ...
    // Run leafHack.js, reserving 24GB of RAM
    ns.run(leafHack, 1, '-r', 24, '-N', 1);
    ...
    while ((ram = ns.getServerMaxRam('home')) < 512 || fileId < darkweb.length) {
        ...
        if (cash >= darkwebFile.cost) {
            if (ns.purchaseProgram(darkwebFile.name)) {
                ...
                ns.run(rootHack);
                await ns.sleep(5000); // Let rootHack settle,
                // New servers to access means more juicy targets to hack
                ns.run(leafHack, 1, '-r', 24, '-N', 5);
            }
        }
